...
class Monitoring(ValidateParams, SetParameters, CalculateOKR, 
    ...
    def fit(self, X, queries=None):
        ...
        for which in self.rules_:
            ...
            # Credit limits
            limits = X_new.loc[cond].groupby("cohort_fr")            .agg({"fnl_cr_lmt":["sum", "count"]})
...
from AssoruleMining import *
from RiskPattern import *


# In[2]:


def get_cohort(cohort, lag=0):
    cohort = date.fromisoformat(f'{cohort}-01') 
    cohort = cohort + timedelta(days=lag*29) 
...
folder ="D:/Users/danusorn.s/OneDrive - KASIKORNBANKGROUP/sharing_dpd_working_team/sharing_processed_data/usl/data_cube"
os.listdir(folder)
...
mapping = {"a_lmt_fpd"    : "flg_06pM01", 
           "a_lmt_61pM03" : "flg_60pM03",
           "a_lmt_91pM04" : "flg_90pM04"}
...
OKR = OKR.loc[OKR["used_in_3mth"]=='ALL', usecols].reset_index(drop=True)
OKR.rename(columns={"credit_seg":"modl_seg", "SA_SE" : "sa_se"}, inplace=True)
OKR["okr_type"] = OKR["okr_type"].apply(lambda x: mapping[x])


# ## Determine risky cohorts

# In[6]:


mature_cohort = "2023-10"
observed_mths = 18
...
start_cohort  = get_cohort(mature_cohort, -observed_mths)
latest_cohort = get_cohort(mature_cohort, -laggings[-1][1])
print("Starting cohort : {}".format(start_cohort))
print("Selected cohort : {}".format(latest_cohort))
...
X.rename(columns={"SA_SE" : "sa_se"}, inplace=True)
modl_seg = {"NTB":"1]NTB", "NTC":"2]NTC", "ETC":"3]ETC"}
X["modl_seg"] = X["modl_seg"].apply(lambda x: modl_seg[x])
X = X.loc[X["cohort_fr"]>=start_cohort].reset_index(drop=True)


# In[10]:


# Replace premature flags with NaN
max_cohort = pd.to_datetime(mature_cohort, format="%Y-%m")
cohorts = pd.to_datetime(X['cohort_fr'], format="%Y-%m")
mobs = ((max_cohort - cohorts).dt.days/30).astype(int)
for col,lag in laggings: X[col] = np.where(mobs<lag, np.nan, X[col])
...
prim_keys  = ["apl_grp_no", "ip_id", "product_tp", "cohort_fr"]
categories = ['age_grp', 'sa_se', 'incm_grp', 'incmv_grp', 
              'src_incm_grp', 'fnl_cr_lmt_grp', 'apl_chnl', 
              'fico_range', 'credit_seg', 'mth_tnr_range']


# In[12]:


onehot = ModifiedEncoder(categories).fit(X)
cat_X  = X[prim_keys].merge(onehot.transform(X), left_index=True, right_index=True)


# Select the latest period of indicator 3

# In[13]:


latest = cat_X["cohort_fr"]==latest_cohort 
latest_X = cat_X.loc[latest].reset_index(drop=True).copy()
latest_y = X.loc[latest, laggings[2][0]].values.astype(int)
print("Number of targets : {:,.0f} ({:.2%})".format(sum(latest_y), np.mean(latest_y)))
...
min_samples = int(np.fmin(len(latest_X)*0.05, 500))
estimator = DecisionTreeClassifier(max_depth=max_depth,
                                   max_features=len(onehot.columns_), 
                                   random_state=0, 
...
TRM = TreeRuleMining(estimator, exclude=False, metric="precision", max_iter=20)
TRM.fit(latest_X[onehot.columns_], latest_y)


# In[16]:


TRM.evaluate(latest_X, latest_y, cumulative=True).fillna(0)
...
for key in TRM.rules.keys():
    print(key)
    print_rule(TRM.rules[key])
    print("\n")


# Adding attained rules to `X`.

# In[18]:


kwds = dict(left_index=True, right_index=True)
new_X = X.merge(TRM.transform(cat_X, TRM.n_rules), **kwds)


# Fit **`Monitoring`** class with `new_X` (with rules).

# In[19]:


FindRisky = Monitoring(OKR, TRM, product="CC").fit(new_X)


# Use **`plot`** to visually observe the trend of all indicators.

# In[20]:


for which in FindRisky.rules_:
    ax = FindRisky.plot(which=which)
    plt.show()


# `results_` (attribute) is a dict with keys as column headers and values as columns, that can be imported into a pandas DataFrame. For multi-period coefficients, the scores for all variables i.e. 3 indicators + {**limit**, **count**} are available in the `results_` dict at the keys ending with that variable’s name ('<var>_Last<period>M').

# In[21]:


results = pd.DataFrame(FindRisky.results_)
results.head(3)
...
file = "Test{}.txt".format(mature_cohort.replace("-",""))
results.to_csv(file, sep="|")


# ## Monitor risky cohorts

# In[23]:


queries = pd.read_csv(file, sep="|", usecols=["name","query"])
queries = dict([k for k in queries.values if k[0] in ["Rule_1", "Rule_3"]])


# Fit **`Monitoring`** class with `X` (without rules). Using `queries`, the algorithm generates rules corresponding to the number of valid keys found in queries. If `queries` is provided, it will override all rules contain inside `estimator`.

# In[24]:


MonitorRisky = Monitoring(OKR, product="CC").fit(X, queries)


# In[25]:


for which in MonitorRisky.rules_:
    ax = MonitorRisky.plot(which=which)
    plt.show()
